document.addEventListener("DOMContentLoaded", function () {
	...
	const counterElement = document.querySelector(".product__info-details-counter");
	const addButton = document.querySelector(".product__info-details-btn-add");
	const subtractButton = document.querySelector(".product__info-details-btn-subtract");

	setupCounter(counterElement, addButton, subtractButton);

	function toggleOptionsVisibility() {
		const toggleButton = document.querySelector('.product__info-select-options');
		const optionsList = document.querySelector('.product__info-options-wrapper');

		// Add a button click event handler
		toggleButton.addEventListener('click', function () {
			// Use toggle to switch visibility class
			optionsList.classList.toggle('product__info-options-wrapper-visibly');
		});
	}
	...
	const checkboxes = document.querySelectorAll('.product__info-options-checkbox input[type="checkbox"]');
	const itemTexts = document.querySelectorAll('.product__info-options-item');

	// Add an event handler to change text styles when the checkbox state changes
	checkboxes.forEach((checkbox, index) => {
		checkbox.addEventListener('change', function () {
			if (this.checked) {
				itemTexts[index].style.color = 'rgb(20, 107, 135)';
			} else {
				itemTexts[index].style.color = ''; // Return default color
			}
		});
	});
});